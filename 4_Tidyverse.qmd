...
```{webr Ayuda sobre los datos de muestra}
***(mtcars)
```
...
```{webr Gráfica de dispersión básica}
...
```{webr Gráfica de dispersión con tendencia}
...
```{webr Gráfica de dispersión con tendencia lineal}
grafica_mpg_vs_wt <-
  ***(data = ***, mapping = aes(x = ***, y = ***)) +
  ***() +
  geom_smooth(color = ***,
...
```{webr Gráfica de dispersión con etiquetas}
grafica_mpg_vs_wt <-
  *** +
  labs(title = ***,
       subtitle = ***,
...
```{webr Gráfica de dispersión con tema}
grafica_mpg_vs_wt <- 
  *** +
  theme_classic()
...
```{webr Gráfica de dispersión coloreada por categorías}
...
```{webr Gráfica de dispersión por facetas}
...
```{webr Ayuda sobre los datos de muestra iris}
help(***)
```

Carga en el entorno los datos de muestra incluidos en *R* llamados `mtcars` usando la función `data` y previsualiza su contenido:

```{webr Cargar datos iris}
***("iris")
...
```{webr Ejercicio diagrama de caja}
# Escribe aquí el código
```
...
```{webr Leer datos sobre precios del oro}
precios_oro <- 
  read_csv(***) %>% 
  ***() %>% 
  ***()
...
```{webr Gráfica de línea básica}
ggplot(precios_oro, aes(x = date, y = united_states_usd)) +
  geom_line()
```

**Ejercicio extra:** 
Repite el gráfico anterior y agrega las etiquetas necesarias para documentar el gráfico y cambia el formato usando el tema `theme_minimal`. Incluye la fuente de los datos en el pie de gráfica.

```{webr Ejercicio línea de tiempo}
# Escribe aquí el código
...
```{webr Guardar archivo de la gráfica}
ggsave(***)
```
