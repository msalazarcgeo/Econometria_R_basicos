...
Installando la biblioteca `rvest` se puede descargar ciertas tablas de internet  vamos a descargar la tabla de [*basketball-reference*](https://www.basketball-reference.com/leagues/NBA_2025_totals.html){target="_blank"}. 
...
* Con la misma tabla del ejercicio anterior obten el número de jugadores que jugaron al menos 60 juegos y tuvieron un porcentage de 3 mayor al 35%. 
...
* Usando la tabla anterior vamos a calcular algunas estadísticas extras con las siguientes fórmulas `trueShooting = PTS / (2 * (FGA + (.44 * FTA)))`,  `effectiveFG  = (FG + (.5 * X3P)) / FGA`  y `shootingDif  = trueShooting - FG.`. Algunas de estas estadísticas ya se encuentran en la tabla, aunque por sus valores estan cortados en la tabla original. 
